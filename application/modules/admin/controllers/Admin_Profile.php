<?php
 ...
 class Admin_Profile extends MX_Controller {
     ...
     function __construct() {
         ...
         $this->data      = [];
         $sessionData = validate_admin_cookie(COOKIE_NAME, 'moso_admin');
         if ($sessionData) {
     ...
     public function admin_profile() {
         ...
         load_views( "profile/profile", $data );
     ...
     public function admin_change_password() {
         try {
             ...
             if ( isset( $postdata ) && !empty( $postdata ) ) {
                 ...
                 $this->form_validation->set_rules( 'password', $this->lang->line( 'new_password' ), 'trim|required' );
                 $this->form_validation->set_rules( 'confirm_password', $this->lang->line( 'confirm_password' ), 'trim|required' );

                 //Checking Form validation
                 if ( $this->form_validation->run() ) {
                     if ( $postdata["password"] === $postdata["confirm_password"] ) {
                         //Checking old password is correct or not
                         $isExists = $this->Common_model->fetch_data( 'moso_admin', 'admin_password',
                         ...
                         if ( isset( $isExists ) && !empty( $isExists ) ) {
                             ...
                             if ( $this->Common_model->update_single( "moso_admin", $userdata, $where ) ) {
                         ...
                         $data['error_message'] = $this->lang->line( 'no_password_match' );
                     }//IF  ENDS
                 }//ELSE 2
             }//IF end
//             print_r($data);die;
         ...
         try {
             ...
             if ( isset( $postdata ) && !empty( $postdata ) ) {
                 ...
                 if ( $this->form_validation->run() ) {//IF 2
                     ...
                     $where     = array ("where" => array ('admin_id' => $this->admininfo['admin_id']));
                     $isSuccess = $this->Common_model->update_single( "moso_admin", $adminData, $where );
