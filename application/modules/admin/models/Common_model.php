...
//        }
    ...
    public function getNotifications($params) {
        ...
        if (!empty($params['startDate']) && !empty($params['endDate'])) {
            $pushStartDate = strtotime($params['startDate']);
            $pushEndDate = strtotime($params['endDate']);
            $this->db->where("created_on >= '" . $pushStartDate . "' AND created_on <= '" . $pushEndDate . "' ");
        }
    ...
    public function terms(){
        $this->db->select("c.description,c.title");
    ...
    /**
     * 
     * @param string $tableName name of the table to fetch data
     * @param array $params filter parameters [id] 
     * @return json data
     */
    public function fetchFromFbase($tableName,$params = array()) {
        if(!empty($tableName)){
            $basePath = "https://moresocial-198800.firebaseio.com/";
            $appSecret = "2LeVZLUMRJRMAFFXN2yC0h9o7ssxDcrXxdCnp9Tu";
            $parameters = ["auth" => $appSecret];
            if(isset($params['id'])){
                $endPoint = $basePath.$tableName.'/'.$params["id"].'/.json?'. http_build_query($parameters);
            }else{
               $endPoint = $basePath.$tableName.'/.json?'. http_build_query($parameters);
            }
            $ch = curl_init();
