...
class Cms_Model extends CI_Model {
    ...
    public function pagelist($limit, $offset, $params) {
        $this->db->select("SQL_CALC_FOUND_ROWS e.*,user.username", False);
        ...
        if (!empty($params['searchlike'])) {
            $this->db->group_start();
            $this->db->like('evt_name', $params['searchlike']);
            $this->db->or_like('id',$params['searchlike']);
            $this->db->group_end();
        }
        if(isset($params['join'])){
            foreach ($params['join'] as $value){
               $this->db->join($value['table'],$value['on'],'INNER');
            }
        }
        ...
        if(!empty($params['time'])){
            $this->db->where('evt_end >', $params['time']);
        }
    ...
    public function count($params){
        if(isset($params['fields'])){
            ...
            $this->db->select("SQL_CALC_FOUND_ROWS count(*) as cnt", False);
        }
        
        if(isset($params['table']) && !empty($params['table'])){
            $this->db->from($params['table'] . ' as e');
        }
        if(isset($params['where'])){
            $this->db->where($params['where']['key'],$params['where']['value']);
        }
        $query = $this->db->get();
