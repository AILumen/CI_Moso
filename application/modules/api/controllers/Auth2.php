...
class Auth2 extends REST_Controller{
        ...
        if(isset($social_type) && !empty($social_type) && isset($device_type) && !empty($device_type)) {
            ...
            if(!empty($userArr)){
                ...
                if($validate === FALSE){
                    ...
                    $errorMsgArr['STATUS'] = FALSE;
                    $errorMsgArr['APICODERESULT'] = $this->lang->line('APIRESULT_SUCCESS');
                    $errorMsgArr['MESSAGE'] = $this->lang->line('ATTEMP_EXCEED_STATUS');
                    $this->response($errorMsgArr);
                }
                //FETCH SESSION INFO 
    ...
    public function signup($postDataArr){
        ...
        if(isset($username) && !empty($username) && isset($password) && !empty($password) && isset($device_type) && !empty($device_type)){
            ...
            $check = $this->Common_model->fetch_data('user', 'userid,status,name,username,email,contact,image,userbio', array('where' => $where), true);
            //check existance
            if(!empty($check)){
                $errorMsgArr = array();
                ...
                $errorMsgArr['STATUS'] = FALSE;
                $errorMsgArr['APICODERESULT'] = $this->lang->line('APIRESULT_SUCCESS');
                $errorMsgArr['MESSAGE'] = ($is_phone) ? $this->lang->line('NUMBER_EXIST') : $this->lang->line('EMAIL_EXIST');
                $this->response($errorMsgArr);
            }
            $insDataArr = array(
                "social_type" => 1,
